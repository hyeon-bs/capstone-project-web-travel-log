...
> 졸업 이후 프로젝트를 직접 재검토하여 v1의 문제점을 보완하고 기능을 확장했습니다.

### 신규 기능

| 기능 | 파일 | 설명 |
|------|------|------|
| 게시글 수정 | `edit.jsp` / `edit_Action.jsp` | 작성자 본인만 수정 가능, 세션 기반 소유자 검증 |
| 게시글 삭제 | `delete_Action.jsp` | 작성자 본인만 삭제 가능 |
| 로그아웃 | `logout.jsp` | 세션 무효화 후 메인 페이지 리다이렉트 |
| 프로필 조회 | `profile.jsp` / `profile_Action.jsp` | 로그인 사용자의 회원 정보 조회 |

### 구조 개선

- **DB 연결 유틸 분리 (`src/db/DBUtil.java`):** 기존에 각 DAO 내부에 하드코딩되어 있던 DB 접속 정보를 외부 `db.properties` 파일 기반으로 분리하여 자격증명 노출 위험을 제거했습니다.
- **세션 기반 접근 제어 강화:** 로그인 없이 내부 페이지에 직접 접근할 경우 `main.jsp`로 강제 리다이렉트되도록 모든 페이지에 세션 검증 로직을 추가했습니다.
